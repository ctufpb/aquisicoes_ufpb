  ...
  async fetch(request, env) {
    ...
    if (url.pathname === '/pncp-link-cache') {
      if (request.method === 'GET') {
        ...
        if (!/^\d{17}$/.test(purchaseKey)) return jsonResponse({ error: 'Chave da compra invÃ¡lida.' }, 400);
        const links = await readPermanentPncpLinks(env.DB, purchaseKey);
        if (!links) return jsonResponse({ error: 'Links do PNCP ainda nÃ£o armazenados.' }, 404);
      ...
      if (request.method === 'POST') {
        ...
        catch { return jsonResponse({ error: 'ConteÃºdo invÃ¡lido.' }, 400); }
        const links = parsePermanentPncpLinks(body);
        if (!links) return jsonResponse({ error: 'Identificadores do PNCP invÃ¡lidos.' }, 400);
        const now = Math.floor(Date.now() / 1000);
        const stored = await writePermanentPncpLinks(env.DB, links, now);
        if (!stored) return jsonResponse({ error: 'NÃ£o foi possÃ­vel armazenar os links do PNCP.' }, 503);
      ...
      return jsonResponse({ error: 'MÃ©todo nÃ£o permitido.' }, 405, { allow: 'GET, POST' });
    }
    ...
    if (url.pathname === '/edital-cache') {
      if (request.method === 'GET') {
        ...
        if (!notice) return jsonResponse({ error: 'Edital ainda nÃ£o armazenado.' }, 404);
        return jsonResponse({
      ...
      if (request.method === 'POST') {
        ...
        if (!notice) return jsonResponse({ error: 'Dados do edital invÃ¡lidos.' }, 400);
        const stored = await writePermanentNotice(env.DB, notice, Math.floor(Date.now() / 1000));
        if (!stored) return jsonResponse({ error: 'NÃ£o foi possÃ­vel armazenar o edital.' }, 503);
    ...
    if (request.method === 'GET' && url.pathname === '/pncp-proxy') {
      ...
      catch { return jsonError('ServiÃ§o oficial temporariamente indisponÃ­vel.', 502); }
